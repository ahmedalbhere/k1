...
async function barberSignup() {
    ...
    try {
        ...
        if (imageFile) {
            const imageRef = storageRef(storage, `barber_images/${user.uid}`);
    ...
    onValue(ref(database, 'barbers'), (snapshot) => {
        ...
        Object.entries(barbers).forEach(([id, barber]) => {
            ...
            barberCard.innerHTML = `
                ${barber.imageUrl ? `<img src="${barber.imageUrl}" class="barber-image" alt="صورة ${barber.name}">` : ''}
                <div class="barber-info">
                    <div class="barber-header">
                        <div class="barber-avatar">${barber.name.charAt(0)}</div>
                        <div class="barber-name">${barber.name}</div>
                    </div>
                    ${barber.location ? `<div class="barber-location"><i class="fas fa-map-marker-alt"></i> ${barber.location}</div>` : ''}
